...
impl CallTraces {
    ...
    pub fn push_call_trace(&mut self, call_trace: InternalCallTrace) {
        if let Some(last) = self.0.last_mut() {
            last.sub_traces.push(call_trace).expect("exceed the call trace limit");
        }
    }

    pub fn prepend_call_trace(&mut self, call_trace: InternalCallTrace) {
        if let Some(first) = self.0.first_mut() {
            first.sub_traces.prepend(call_trace).expect("exceed the call trace limit");
        }
    }

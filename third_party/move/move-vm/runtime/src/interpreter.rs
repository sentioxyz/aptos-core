            ...
            match exit_code {
                ...
                Ok(ExitCode::CallClosure(_sig_idx)) => {
                    ...
                    if callee.is_native() {
                        ...
                        let mut inputs = vec![];
                        if let Ok(vals) = self.operand_stack.last_n(callee.param_tys().len()) {
                            for val in vals {
                                if let Ok(v) = val.copy_value(1, Some(DEFAULT_MAX_VM_VALUE_NESTED_DEPTH)) {
                                    inputs.push(v);
                                }
                        ...
                        let _ = self.call_traces.push(InternalCallTrace {
                            ...
                            inputs: self.decode_move_values(gas_meter, traversal_context, callee.param_tys(), &callee.ty_args, inputs),
                            outputs: vec![],

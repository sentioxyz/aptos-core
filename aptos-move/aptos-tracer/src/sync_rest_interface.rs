...
impl AptosTracerInterface for RestTracerInterface {
    ...
    fn get_committed_transactions(
        ...
        let txns_and_infos = std::thread::spawn(move || {
        ...
        Ok((txns_and_infos.0, txns_and_infos.1))
    }
    ...
    fn get_block_first_version(&self, version: Version) -> Result<Option<Version>> {
        let base_url = self.client.path_prefix_string();

        std::thread::spawn(move || {
            let url = format!("{}blocks/by_version/{}", base_url, version);
            let resp: serde_json::Value = reqwest::blocking::get(&url)?.json()?;
            let first_version = resp["first_version"]
                .as_str()
                .ok_or_else(|| anyhow!("Missing first_version in block response"))?
                .parse::<u64>()?;

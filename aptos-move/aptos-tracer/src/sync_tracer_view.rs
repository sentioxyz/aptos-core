...
pub trait AptosTracerInterface: Sync {
    ...
    /// Get the first version of the block containing the given version.
    /// Returns None if not supported (e.g. DB mode).
    fn get_block_first_version(&self, _version: Version) -> Result<Option<Version>> {
        Ok(None)
    }
}

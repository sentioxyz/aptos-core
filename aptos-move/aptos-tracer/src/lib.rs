...
// some methods are missing from AptosValidatorInterface
//
...
impl SyncAptosTracer {
    ...
    pub fn trace_transaction(
        ...
        // Compute transaction_index from block info for accurate AuxiliaryInfo
        let transaction_index = self.debugger
            .get_block_first_version(txn_data.version)?
            .map(|first_version| (txn_data.version - first_version) as u32);

        let call_traces = match txn {
            Transaction::UserTransaction(user_txn) => {
                let trace_result = AptosVM::get_call_trace(
                    &state_view,
                    &user_txn,
                    user_txn.max_gas_amount(),
                    transaction_index,
                ...
                let call_trace = trace_result.call_traces;
                ...
                if !module_id.is_none() {
                    let unwrapped_module_id = module_id.unwrap();
                    let package_registry = self.debugger.get_package_registry(
                        *unwrapped_module_id.clone().address(), Version::from(txn_data.version)).unwrap_or_default();
                    let matched_package = match package_registry {
                        None => {None}
                        Some(registry) => {
                            // find the module in the package registry
...
impl CallTraceWithSource {
    ...
    pub fn from_modules(call_trace: InternalCallTrace, modules_map: &HashMap<String, ModuleCompilation>) -> Self {
        ...
        let offset_to_loc =  |account: String, module_name: String, func_idx: FunctionDefinitionIndex, code_offset: CodeOffset| -> Option<Location> {
            ...
            if module_info.source_map.len() == 0 || module_info.source.len() == 0 {
                return None;
            }
            let mut files = Files::new();
            let file_id = files.add(module_name.to_owned(), module_info.source.clone());
            let source_map = hex::decode(module_info.source_map.clone().strip_prefix("0x").unwrap()).unwrap();
...
pub struct PackageCompilation {
    name: String,
    #[serde(rename = "moduleWithoutCode")]
    module_without_code: Option<Vec<String>>,

...
impl AptosVM {
    ...
    pub fn get_call_trace(
        ...
        // Unlike the production execution path, call tracing does not run
        // `validate_signed_transaction` or the account-abstraction prologue before executing the
        // payload. Capping the initial balance at `max_aa_gas` here therefore has no matching
        // validation phase after which the rest of the transaction budget can be injected.
        let initial_balance = max_gas_amount.into();
